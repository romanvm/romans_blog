...
var YouTubeDialog = {
	...
	insert : function() {
		...
		if(f.youtubeID.value.match(new RegExp("//www.youtube.com/","g"))){
			//Replace url
			var youtubeuri = f.youtubeID.value.replace(f.youtubeID.value,"//www.youtube.com/v/"+YouTubeDialog.videovalue(f.youtubeID.value));
			//Construct URL
			var constructUri = youtubeuri+options;
		}else if(f.youtubeID.value.match(new RegExp("http://youtu.be/","g"))){
			//Replace url
			var youtubeuri = f.youtubeID.value.replace('//youtu.be/',"//www.youtube.com/v/");
			//Construct URL
			var constructUri = youtubeuri+options;
		}
