...
Login into the admin panel, open "Sites" section and enter ``127.0.0.1:8000`` as :guilabel:`Domain name`
(:guilabel:`Display name` can be any). This is needed so that links from the admin panel to your site pages work properly.

You can also run :file:`populate_db.py` script to populate your test site with "Lorem Ipsum" posts and pages.

To test file upload create a folder named :file:`media` in the project's folder,
then create :file:`uploads` sub-folder inside it.
...
Create a folder that will be served by your web-sever under :file:`/media/` path.
Then create a sub-folder :file:`/uploads/` inside it. This subfolder must be accessible for writing to the web-server
user.

By default the main project's :file:`settings.py` module tries to import :file:`local_settings.py` module with production
settings. So you must create such file along with the main :file:`settings.py`. As the absolute minimum,
:file:`local_settings.py` must define your production ``SECRET_KEY`` and ``DATABASES`` variables. You can use
...
:file:`local_settings.py` as an "anti-pattern", but in my opinion that is too harsh and using a "local" Python
...
Also set :guilabel:`Domain name` in the admin panel (see above) to your site's actual domain name.
...
``HAYSTACK_SIGNAL_PROCESSOR = 'haystack.signals.RealtimeSignalProcessor'`` in the project's :file:`settings.py`
...
For spellcheck to work properly in TinyMCE editor you may need to add spelling dictionaries for your languages.
Read `Pyenchant`_ package docs for more info.
